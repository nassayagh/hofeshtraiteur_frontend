...
<template>
    <VDialog ref="dialog" v-model="modal" v-model:return-value="date" persistent width="290px">
        ...
        <template #default="{ isActive }">
            <VLocaleProvider :locale="$i18n.locale">
                <VDatePicker v-model="date" hide-header>
                    <template #actions>
                        <VBtn color="primary" @click="modal = false">
                            {{ $t('Annuler') }}
                        </VBtn>
                        <VSpacer />
                        <VBtn variant="elevated" color="primary" @click="modal = false">
                            {{ $t('OK') }}
                        </VBtn>
                    </template>
                </VDatePicker>
