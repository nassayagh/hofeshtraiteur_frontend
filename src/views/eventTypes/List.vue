...
// theme breadcrumb
const page = ref({ title: 'Gestions des événements' });
        ...
        if (valid) {
            ...
            store
                ...
                .then((response) => {
                    ...
                    snackbarStore.showSuccess(t('Événement sauvée avec succès'));
                })
                .catch((error) => {
                    saving.value = false;
...
function deleteItemConfirm() {
    ...
    snackbarStore.showSuccess(t('Événement supprimé avec succès'));
}

//Computed Property
const formTitle = computed(() => {
    return editedIndex.value === -1 ? t('Nouvel événements') : t('Modifier la événements');
});
