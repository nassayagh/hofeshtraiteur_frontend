...
const props = defineProps({
    ...
    title: t('Service'),
    buttonText: { type: String, default: t('Modifier La Prestation') }
});
...
function save(values: any, { setErrors }: any) {
    ...
    refForm.value?.validate().then(({ valid }) => {
        if (valid) {
            saving.value = true;
            store
                .addItem({ ...item.value, event_date: formatedDemandDate.value, comment: editor.value?.getHTML() })
                .then((response) => {
                    ...
                    dialogEdit.value = false;
                    //item.value = { customer: {} };
                    snackbarStore.showSuccess(t('Prestation enregistrée avec succès'));
                    fetchEventTypes();
                    fetchDemands();
...
const formTitle = computed(() => {
    return item.value.id == null ? 'Nouvelle prestation' : 'Modifier la prestation';
});
