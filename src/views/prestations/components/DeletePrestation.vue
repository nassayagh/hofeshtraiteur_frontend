    ...
    <v-dialog v-model="dialog" max-width="500px">
        <template v-slot:activator="{ props }">
            <v-btn class="ml-2" dark v-bind="props" variant="outlined" color="primary" @click="loading = false"
                >{{ $t('Supprimer la prestation') }}
            </v-btn>
        </template>
        <v-card>
            <v-card-title class="text-h5 text-center py-6">{{ $t('Supprimer la prestation') }}</v-card-title>
            <v-divider />
            <v-card-text>
                <v-row>
                    <v-col cols="12" class="text-center">
                        {{ $t('Êtes-vous sûr de vouloir Supprimer la prestation?') }}
                    </v-col>
                </v-row>
            </v-card-text>
            <v-divider />
