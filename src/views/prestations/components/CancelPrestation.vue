...
const title = computed(() => `${t('Annuler la prestation')}`);
function validateItemConfirm() {
    loading.value = true;
    store
        .cancelItem(item.value.id, comment.value)
        .then((response) => {
            item.value = response.data;
            dialog.value = false;
            snackbarStore.showSuccess(t('Prestation annulée avec succès'));
...
<template>
    <v-dialog v-model="dialog" max-width="500px">
        <template v-slot:activator="{ props }">
            <v-btn class="ml-2" dark v-bind="props" variant="outlined" color="primary" @click="loading = false"
                >{{ $t('Annuler la prestation') }}
            </v-btn>
        </template>
        <v-card>
            <v-card-title class="text-h5 text-center py-6">{{ $t('Annuler la prestation') }}</v-card-title>
            <v-divider />
            <v-card-text>
                <v-row>
                    <v-col cols="12" class="text-center">
                        {{ $t('Êtes-vous sûr de vouloir Annuler la prestation?') }}
                    </v-col>
                    <v-col cols="12">
