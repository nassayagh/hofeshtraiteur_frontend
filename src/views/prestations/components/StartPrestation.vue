...
const title = computed(() => `${t('Lancer la prestation')}`);
function validateItemConfirm() {
    loading.value = true;
    store
        .startItem(item.value.id)
        .then((response) => {
            item.value = response.data;
            dialog.value = false;
            snackbarStore.showSuccess(t('Prestation lancée avec succès'));
...
<template>
    <v-dialog v-model="dialog" max-width="500px">
        <template v-slot:activator="{ props }">
            <v-btn dark v-bind="props" variant="elevated" color="primary" @click="loading = false">{{ $t('Lancer la prestation') }} </v-btn>
        </template>
        <v-card>
            <v-card-title class="text-h5 text-center py-6">{{ $t('Lancer la prestation') }}</v-card-title>
            <v-divider />
            <v-card-text>
                <v-row>
                    <v-col cols="12" class="text-center">
                        {{ $t('Êtes-vous sûr de vouloir Lancer la prestation?') }}
                    </v-col>
                </v-row>
            </v-card-text>
            <v-divider />
